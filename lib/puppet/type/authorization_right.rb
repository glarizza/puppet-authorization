Puppet::Type.newtype(:authorization_right) do

  desc "Manage rights in the Mac OS X authorization database. See the
  ...
  newparam(:name, :namevar => true) do
    desc "The name of the right to be managed.  Corresponds to `key` in
    Authorization Services. The key is the name of a rule. A key uses the same
    naming conventions as a right. The Security Server uses a rule's key to
    match the rule with a right.  Wildcard keys end with a '.'. The generic
    rule has an empty key value.  Any rights that do not match a specific rule
  ...
  newproperty(:auth_class) do
    ...
    newvalues([:user, :'evaluate-mechanism', :allow, :deny, :rule])
  end
